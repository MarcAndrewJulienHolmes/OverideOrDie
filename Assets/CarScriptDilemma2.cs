...
public class CarScriptDilemma2 : MonoBehaviour {
    ...
    void Start()
    {
        ...
        objBus= (GameObject)GameObject.Find("Bus_yellow");
        ...
        wr.WriteLine("Dilemma 3," + System.DateTime.Now.ToString());
        wr.Flush();
    ...
    void Update()
    {
        ...
        if (objCamera.transform.position.z < -26.0F && decided == 1.0F)
        {
            ...
            if (Horn.activeSelf == false)
            {
                ...
                audio = GetComponent<AudioSource>();
                audio.PlayOneShot(HornSound, 0.35F);
                ObjLook_left.SetActive(true);
                PedestrianWarningSymbol2.gameObject.SetActive(false);
            ...
            if (objBus.transform.position.x < -61.5f)
            {
                Debug.Log("here5"); 
                wr.WriteLine("Stop," + System.DateTime.Now.ToString());
                wr.Flush(); 
                SceneManager.LoadScene("BIKE_OutcomeStop", LoadSceneMode.Single);
...

        {
            if (objCamera.transform.position.z < -26.5F && decided == 1.5F)
                ...
                SceneManager.LoadScene("BIKE_OutcomeKill", LoadSceneMode.Single);
