...
public class CarScriptDilemma3a : MonoBehaviour {
    ...
    void Start()
    {
        ...
        objTrain = (GameObject)GameObject.FindWithTag("Train");
        ...
        Train_horn = (GameObject)GameObject.Find("Train_horn");
        Train_horn.gameObject.SetActive(false);
        ...
        wr.WriteLine("Dilemma 5," + System.DateTime.Now.ToString());
        wr.Flush();
    ...
    void Update()
    {
        ...
        {
            audio = GetComponent<AudioSource>();
            audio.PlayOneShot(Pedestrian_warning_dilemma1, 2.5F);
        ...
        if (objCamera.transform.position.x > -53.75F && stuck == false)
        {
           ...
           //Train_horn.gameObject.SetActive(true);
            audio = GetComponent<AudioSource>();
            audio.PlayOneShot(Train_sound, 0.35F);
            stuck = true;
        ...
        if (stuck == true && decided==1.0F)
        {
            ...
            if (objTrain.transform.position.z >-9.0f)
            {
                wr.WriteLine("Stop," + System.DateTime.Now.ToString());
                wr.Flush();
                SceneManager.LoadScene("Train_Outcomestop", LoadSceneMode.Single);
            }
...

        {
            if (objCamera.transform.position.x > -53.5F && decided == 1.5F) 
                ...
                SceneManager.LoadScene("Train_Outcomekill", LoadSceneMode.Single);
